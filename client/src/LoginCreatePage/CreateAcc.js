...
function CreatAccForm(props) {
  ...
  const handleChange = (e) => {
    ...
    if (/\s/.test(value)) {
      setWhiteSpaces(true);
      setUserValidation("Username cannot have spaces");
  ...
  const handleFeed = () => {
    if (AccountCreation === false) return;
    else
      return (
      ...
      <div className="Create">
        ...
        <div className="Create-btnc">
          <div className="popup-bg">
            ...
            >
              <span id="Text-gradient">Create Account</span>
            </button>
          </div>
        </div>
      </div>
...
function Create() {
  ...
  const setPic = () => {
    window.location.assign(
      process.env.REACT_APP_baseServerurl + `/create/${Username}/avatar`
    );
  };
